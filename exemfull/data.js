...
const elCard = document.querySelector(".card-body");
...
elSearch.addEventListener("input", (e) => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].name.includes(element.value)) {
      searchArr.push(data[i]);
    }
  }
  renderPosts(searchArr);
});
// delet post
...
function renderPosts(array) {
  ...
  array.forEach((post) => {
    ...
    newRow.className = "col-3";
    newRow.innerHTML += ` 
	<div class="card mb-3">
	<div class="card-body">
  <h1 class="card-text text-center">PostId
	  ${post.postId}
	  </h1>
	  <h5 class="card-title text-center">${post.name}</h5>
    <a href="#" class="link-success">${post.email}</a>
	  <p class="card-text text-capitalize text-center">
	  ${post.body}
	  </p>
	  <button type="button" class="btn btn-outline-danger btn-delete" id="delete" data-id="${post.id}">Delete</button>
    </div>
    </div> 
    `;
    elRow.appendChild(newRow);
  });
}
renderPosts(data);
...
elSelect.addEventListener("change", () => {
  ...
  if (type == "All select") {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (element.postId == type) {
        filterArr.push(element);
      }
    }

    renderPosts(filterArr);
  }
});

function rederPostId(postArr, p) {
  let postIdArr = [];
